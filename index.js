...
const TOKEN = "8796146859:AAGY4Sg19to3NHMb1KV0p6p8RR5R6xazHmo";
...
bot.on("message", async (msg) => {
    ...
    if (sessions[chatId]?.createRepoMode) {
        ...
        try {
            ...
            bot.sendMessage(chatId, "❌ Repo create failed");
        }
...
bot.on("callback_query", async (q) => {
    ...
    if (data.startsWith("remove_")) {
        ...
        bot.sendMessage(chatId, `❌ Removed user: ${userId}`);
        bot.sendMessage(userId, "🚫 Your access has been removed by admin");
    }

    // REFRESH REPOS
    if (data === "refresh_repos") {
        ...
        try {
            ...
            bot.sendMessage(chatId, "❌ Failed to refresh repos");
        }
    }
        ...
        bot.sendMessage(chatId, `📁 Repo: ${repo}`, {
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: "🔄 Refresh Repos", callback_data: "refresh_repos" }],
    ...
    if (data === "create_repo") {
        ...
        bot.sendMessage(chatId, "📦 Send new repo name:");
    }
    ...
    // BACK
    if (data === "back") {
        bot.sendMessage(chatId, "🔙 Use /start");
    }
